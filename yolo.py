...
class YOLO(object):
    ...
    def detect_image(self, image):
        start = timer()
        global anser #測試:新增全域變數anser
        anser='null' #測試:設定anser預設內容'null'
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            label = '{} {:.2f}'.format(predicted_class, score) #測試:predicted_class是判斷出的物體
            if (predicted_class=='knife'):
                anser='knife'   #測試
...
def firstRun(): #初始化運行
    global yolo #設定全域變數yolo、path
    global path
    yolo=YOLO()
    path = './imgtest/ltuschool.jpg'#設定圖片位置與檔名
